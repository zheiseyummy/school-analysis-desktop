...
export function getJuniorQualityClasses() { return request({ url: "/api/v1/quality/classes", method: "get" }); }
export function getQualityClassStudents(clazzId: number) { return request({ url: `/api/v1/quality/class/${clazzId}`, method: "get" }); }
...
export function importQualityWorkbook(clazzId: number, file: File) {
  ...
  return request({ url: "/api/v1/quality/import", method: "post", params: { clazzId }, data: formData, headers: { "Content-Type": "multipart/form-data" } });
}
export function exportQualityWorkbook(clazzId: number) {
  return request({ url: "/api/v1/quality/export", method: "get", params: { clazzId }, responseType: "arraybuffer" });
}
